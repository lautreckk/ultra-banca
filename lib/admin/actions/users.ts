...
export interface UserFinancials {
  ...
  promotor: {
    ...
    comissao_deposito_pct: number | null;
    comissao_perda_pct: number | null;
    total_comissao_gerada: number;
  } | null;
}

export async function getUserFinancials(userId: string): Promise<UserFinancials | null> {
  try {
    await requireAdmin();
    const supabase = await createClient();
    const platformId = await getPlatformId();
    ...
    const totalDepositado = depositosResult.data?.reduce((s, r) => s + (Number(r.valor) || 0), 0) || 0;
    const totalApostado = apostasResult.data?.reduce((s, r) => s + (Number(r.valor_total) || 0), 0) || 0;
    const totalGanhos = ganhosResult.data?.reduce((s, r) => s + (Number(r.premio_valor) || 0), 0) || 0;
    const totalSaques = saquesResult.data?.reduce((s, r) => s + (Number(r.valor) || 0), 0) || 0;
    const lucroCasa = totalDepositado - totalGanhos - totalSaques;

    let promotor: UserFinancials['promotor'] = null;
    if (promotorResult.data?.promotor_id) {
      const p = promotorResult.data.promotores as { nome?: string; codigo_afiliado?: string; comissao_deposito_percentual?: number; comissao_perda_percentual?: number } | null;
      const { data: comissoes } = await supabase
        .from('promotor_comissoes')
        .select('valor_comissao')
        .eq('promotor_id', promotorResult.data.promotor_id)
        .eq('user_id', userId);

      const totalComissao = comissoes?.reduce((s, r) => s + (Number(r.valor_comissao) || 0), 0) || 0;

      promotor = {
        nome: p?.nome || 'N/A',
        codigo_afiliado: p?.codigo_afiliado || '',
        comissao_deposito_pct: p?.comissao_deposito_percentual ?? null,
        comissao_perda_pct: p?.comissao_perda_percentual ?? null,
        total_comissao_gerada: totalComissao,
      };
    }

    return { total_depositado: totalDepositado, total_apostado: totalApostado, total_ganhos: totalGanhos, total_saques: totalSaques, lucro_casa: lucroCasa, promotor };
  } catch (e) {
    console.error('getUserFinancials error:', e);
    return null;
  }
}
