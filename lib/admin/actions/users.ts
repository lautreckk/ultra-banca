...
export async function getUserFinancials(userId: string): Promise<UserFinancials | null> {
  ...
  const [depositosResult, apostasResult, ganhosResult, saquesResult, promotorResult] = await Promise.all([
    ...
    supabase
      ...
      .select('promotor_id, promotores(nome, codigo_afiliado, comissao_deposito_percentual, comissao_perda_percentual)')
      .eq('user_id', userId)
      .single(),
  ]);
  ...
  if (promotorResult.data?.promotor_id) {
    const p = promotorResult.data.promotores as any;
