...
interface SeninhaWizardClientProps {
...
// ── Main Wizard ────────────────────────────────────────
export function SeninhaWizardClient({ modalidades }: SeninhaWizardClientProps) {
  ...
  const REQUIRED_NUMBERS = 14;
  const NUMBER_POOL = 60;
      ...
      <div className="w-full max-w-md bg-[#111318] min-h-[100dvh] shadow-xl flex flex-col relative overflow-hidden">
        ...
        <header className="sticky top-0 z-40 bg-[#1A202C] px-4 shrink-0">
          <div className="flex h-12 items-center justify-between">
            ...
            <span className="text-base font-bold text-white">SENINHA</span>
            <button
              className="flex h-11 w-11 items-center justify-center rounded-lg active:bg-white/10"
              aria-label="Menu"
            >
                ...
                <div key={s} className="flex-1">
                  <div
                    className={cn(
                      ...
                      i <= STEPS.indexOf(step) ? 'bg-blue-500' : 'bg-zinc-700/40'
                    )}
                  />
                </div>
              ))}
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          <div key={step}>
            ...
            {step === 'data' && (
              <div className="p-4">
                <h1 className="text-2xl font-bold text-blue-500 mb-1" style={{ fontFamily: 'serif' }}>
                  SENINHA
                </h1>
            ...
            {step === 'modalidade' && (
              <div className="p-4">
                <h2 className="text-xl font-bold text-white mb-1">SENINHA</h2>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {modalidades.map((mod) => (
                    ...
                    >
                      ...
                      <div className="text-blue-500 text-xs mt-1">{mod.multiplicador}x</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col">
                <div className="p-4 border-b border-zinc-700/40 bg-[#1A1F2B]">
                  <div className="flex justify-between items-start">
                    <h2 className="text-lg font-bold text-white">SENINHA</h2>
                    <div className="text-right">
                      <div className="text-blue-500 font-bold text-sm">{selectedModalidade?.nome}</div>
                    ...
                    {Array.from({ length: NUMBER_POOL }, (_, i) => i + 1).map((num) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            'w-10 h-10 rounded-full flex items-center justify-center text-xs font-medium transition-all active:scale-[0.95]',
                            isSelected
                              ? 'bg-[#111318] text-white ring-2 ring-blue-500'
                              : isInPalpite
                ...
                <div className="p-4 flex gap-3 mt-auto">
                  ...
                  <button
                    ...
                    className="flex-1 h-14 bg-blue-500 rounded-xl font-bold text-white disabled:opacity-50 active:scale-[0.98] transition-all"
                  >
                    Avancar
                  </button>
                </div>
            ...
            {step === 'valor' && (
              <div className="p-4">
                <div className="flex justify-between items-start mb-4">
                  <h2 className="text-xl font-bold text-white">SENINHA</h2>
                  <div className="text-right">
                    <div className="text-blue-500 font-bold text-sm">{selectedModalidade?.nome}</div>
              ...
              <div className="flex flex-col">
                    ...
                    {sorteioDays.map((day) => (
                      ...
                      >
                        <div className={cn(
                          ...
                          selectedDias.includes(day.dateStr) ? 'bg-blue-500 border-blue-500' : 'border-zinc-600'
                        )}>
                ...
                <div className="p-4 mt-auto">
                  <button
                    ...
                    className="w-full h-14 bg-blue-500 rounded-xl font-bold text-white disabled:opacity-50 disabled:bg-zinc-700 disabled:text-zinc-400 active:scale-[0.98] transition-all"
                  >
                    ...
                    <div className="flex justify-center mt-2">
                      {config.logo_url ? (
                        ...
                        <div className="w-20 h-16 bg-[#111318] rounded-xl flex items-center justify-center">
                          <span className="text-blue-500 font-bold text-xs text-center">{config.site_name}</span>
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    {selectedDias.map((dia, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm text-zinc-300 mb-1">
                        ...
                        <span className={isConfirmed ? 'text-green-600' : ''}>
                          SORTEIO MEGA - {sorteioDays.find((d) => d.dateStr === dia)?.dateLabel || dia}
                        </span>
                ...
                {!isConfirmed ? (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="h-14 bg-[#3B82F6] rounded-xl font-semibold text-white flex items-center justify-center gap-2 disabled:opacity-50 active:scale-[0.98] transition-all"
                    >
                      {isLoading ? (
                        <>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={handleShare}
                        className="h-14 bg-[#3B82F6] rounded-xl font-semibold text-white flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
                      >
