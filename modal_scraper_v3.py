"""
Ultra Banca - Scraper de Resultados v3
Otimizado: requests primeiro, Firecrawl somente como fallback pago
"""
...
app = modal.App("ultra-banca-scraper-v3")
...
def scrape_boasorte_requests(data: str) -> list:
    ...
    try:
        ...
        for header in soup.find_all(["h2", "h3", "h4", "strong", "p"]):
            ...
            horario_match = re.search(r'(\d{1,2})\s*(?:[h:H]|hs|horas)\s*(\d{2})?', header_text)
            if not horario_match:
                continue
...
# =============================================================================
# FONTE 1: RESULTADOFACIL VIA REQUESTS (METODO PRIMARIO v3)
# =============================================================================

def scrape_resultadofacil_requests(url: str, estado: str, data: str, banca: str) -> list:
    """
    Scrape ResultadoFacil usando requests direto (método primário no v3)
    """
    import requests
    from bs4 import BeautifulSoup
...
# =============================================================================
# FUNCAO PRINCIPAL v3: requests → PortalBrasil → Firecrawl (fallback)
# =============================================================================
...
def scrape_estado(estado: str, data: Optional[str] = None) -> dict:
    """
    Scrape otimizado v3 - ordem invertida para economia de créditos:
    ...
    print(f"\n{'='*60}")
    print(f"[{estado}] 🎯 SCRAPE v3 - {config['banca']} - {data_scrape}")
    print(f"{'='*60}")
    ...
    if estado == "BS":
        ...
        return {
            "estado": estado,
            "banca": config["banca"],
            "url": f"https://lookgoias.com/boa-sorte-loterias-{data_scrape}",
            "resultados": resultados,
            "fonte_utilizada": fonte_utilizada,
            "tentativas": tentativas,
            "creditos_firecrawl": creditos_firecrawl,
    ...
    if not resultados:
        ...
        try:
            ...
            if not api_key:
                ...
                # v3: pede só HTML (1 crédito), sem wait_for nem markdown
...
def scrape_todos_v3(data: Optional[str] = None, estados: Optional[list] = None) -> dict:
    """
    Scrape todos os estados usando v3 (requests primeiro)
    """
    ...
    print(f"=== Scrape V3 (requests-first) iniciado: {data_scrape} ===")
    ...
    print(f"\n=== Scrape V3 finalizado: {resultado_final} ===")
...
def scrape_scheduled():
    """
    Scrape agendado v3 - roda a cada 30 minutos nos horários relevantes.
    Otimizações:
    ...
    print(f"=== Scrape V3 agendado: {agora_brasilia().strftime('%Y-%m-%d %H:%M:%S')} BRT ===")
            ...
            if aposta_ganhou:
                ...
                if user_id and valor_premio > 0:
                    try:
                        ...
                        print(f"  Payout completo para {aposta['id'][:8]}, novo saldo: R${novo_saldo:.2f}")
                        ...
                        print(f"  Erro ao processar payout {aposta['id'][:8]}: {type(e).__name__}: {e}")
                        import traceback
                        traceback.print_exc()
                ...
                if todas_expiraram and loterias_sem_resultado:
                    # Reembolso direto via tabelas (evita RPC com updated_at)
                    try:
                        ...
                        if valor_reembolso > 0:
                            ...
                            supabase.table("transactions").insert({
                                ...
                                "metadata": {"reason": f"Resultado não disponível após 12h: {loterias_str}", "bet_id": aposta["id"]},
                            }).execute()
                        reembolsado += 1
...
def scrape_ultimos_dias(dias: int = 7) -> dict:
    """
    Scrape dos últimos N dias para todos os estados (v3 - requests primeiro)
    """
    from supabase import create_client
    import time
    ...
    print(f"\n{'='*70}")
    print(f"SCRAPE V3 DOS ÚLTIMOS {dias} DIAS")
    print(f"{'='*70}\n")
    ...
    print(f"\n{'='*70}")
    print(f"RESUMO FINAL V3 - {dias} DIAS")
    print(f"{'='*70}")
...
def main(
    ...
    """
    Comandos v3 (requests-first, Firecrawl como fallback):
    ...
    Exemplos:
        modal run modal_scraper_v3.py --comando scrape --estado MG --data 2026-01-30
        modal run modal_scraper_v3.py --comando todos --data 2026-01-29
        modal run modal_scraper_v3.py --comando historico --dias 7
        modal run modal_scraper_v3.py --comando verificar --data 2026-01-29
    """
    if comando == "scrape":
        print(f"\n{'#'*70}")
        print(f"# SCRAPE V3 (requests-first): {estado} - {data or 'hoje'}")
        print(f"{'#'*70}\n")
        ...
        print(f"Scraping V3 todos os estados para {data or 'hoje'}...")
        resultado = scrape_todos_v3.remote(data)
        print(f"\nResultado: {resultado}")

    elif comando == "historico":
        print(f"\n{'#'*70}")
        print(f"# SCRAPE V3 HISTÓRICO: ÚLTIMOS {dias} DIAS")
        print(f"{'#'*70}\n")
